...
function getCorners(char)
    ...
    local sx, sy, sz = sz.X/2, sz.Y/2, sz.Z/2
    return {
...
function updateESP()
    ...
    for _,plr in ipairs(Players:GetPlayers()) do
        ...
        if hum and hum.Health > 0 and char:FindFirstChild("HumanoidRootPart") then
            ...
            local Ls,tname,bar,num = tbl.L, tbl.name, tbl.lifebar, tbl.lifenum
            local v=getCorners(char)
            if not v then for i=1,4 do Ls[i.."H"].Visible=false Ls[i.."V"].Visible=false end tname.Visible=false bar.Visible=false num.Visible=false continue end
            ...
            if not(u1 and u2 and u3 and u4) then for i=1,4 do Ls[i.."H"].Visible=false Ls[i.."V"].Visible=false end tname.Visible=false bar.Visible=false num.Visible=false continue end
            local L=L_SIZE
            Ls["1H"].From=pTL Ls["1H"].To=pTL+Vector2.new(L,0) Ls["1V"].From=pTL Ls["1V"].To=pTL+Vector2.new(0,L) Ls["1V"].Color=Color3.fromRGB(0,255,0)
            Ls["2H"].From=pTR Ls["2H"].To=pTR-Vector2.new(L,0) Ls["2V"].From=pTR Ls["2V"].To=pTR+Vector2.new(0,L) Ls["2V"].Color=Color3.fromRGB(255,255,0)
            Ls["3H"].From=pBR Ls["3H"].To=pBR-Vector2.new(L,0) Ls["3V"].From=pBR Ls["3V"].To=pBR-Vector2.new(0,L) Ls["3V"].Color=Color3.fromRGB(255,255,0)
            Ls["4H"].From=pBL Ls["4H"].To=pBL+Vector2.new(L,0) Ls["4V"].From=pBL Ls["4V"].To=pBL-Vector2.new(0,L) Ls["4V"].Color=Color3.fromRGB(0,255,0)
            for i=1,4 do Ls[i.."H"].Visible=toggles.box Ls[i.."V"].Visible=toggles.box end
            tname.Position=Vector2.new((pTL.X+pTR.X)/2, pTL.Y-16) tname.Text=plr.Name tname.Visible=toggles.name
            if toggles.life then
                ...
                bar.From = barTop bar.To = filledEnd bar.Visible=true
                num.Position=Vector2.new(barTop.X-14,barBot.Y)
                num.Text=math.floor(ratio*100).."%"
                num.Visible=true
            else bar.Visible=false num.Visible=false end
        end
    end
end
...
Tabs.Mira:AddSlider("SliderFOV", { Title = "Tamanho do FOV", Min = 50, Max = 500, Default = FOV_Size, Rounding = 0,
